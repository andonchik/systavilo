 ...
 * perex_calendars.js
 * perex_flashnews.js
 * Generated by MagicMin: 2020-12-28 12:00:02
 */
...
return null;};})(jQuery);function _toConsumableArray(arr){return _arrayWithoutHoles(arr)||_iterableToArray(arr)||_nonIterableSpread();}
...
PEREX.sendAdblockGaEvents();});});PEREX.registerInitHook(function(){jQuery('#article-fb-share a.share').click(function(e){e.preventDefault();if(typeof FB==="object"){FB.ui({method:'share',href:location.href},function(response){if(typeof ga==="function"){ga('send','event','Share','Facebook','article button');ga('b.send','event','Share','Facebook','article button');}else if(typeof gtag==="function"){gtag('event','share',{'event_category':'Facebook','event_label':'article button'});}});}else{var leftVal=screen.width / 3;var topVal=screen.height / 5;window.open("http://www.facebook.com/sharer.php?u="+encodeURIComponent(location.href)+"&t="+encodeURIComponent(document.title),"sharer","toolbar=0,status=0,width=626,height=436,left="+leftVal+",top="+topVal);}});});PEREX.registerInitHook(function(){jQuery('.listing-filter select').change(function(){if(jQuery(this).val().trim()!==""){window.location.href=jQuery(this).val();}});});PEREX.registerInitHook(function(){jQuery('#arrow-top').click(function(e){e.preventDefault();jQuery('html, body').animate({scrollTop:'0px'},550);});});PEREX.registerInitHook(function(){if(typeof LazyLoad==='function'){window.lazyLoadInstance=new LazyLoad({elements_selector:'.lazy',data_src:'original',threshold:800});}
...
var $modalSms=$('<div class="modal fade" id="modal-supportPravda-sms" tabindex="-1" role="dialog">\n    <div class="modal-dialog" role="document">\n        <div class="modal-content">\n            <div class="modal-header">\n                <button type="button" class="close" data-dismiss="modal" aria-label="Zavrieť"><span aria-hidden="true">&times;</span></button>\n                <h4 class="modal-title">\n                    Podporujete kvalitnú žurnalistiku\n                </h4>\n            </div>\n            <div class="modal-body">\n                <p>\n                    Pošlite SMS s textom <b>PRAVDA X</b> (kde X je suma príspevku) na číslo <b>8877</b>.\n                </p>\n                <ul>\n                    <li data-amount="1">\n                        <b>PRAVDA 1</b><span>1&euro;</span>\n                    </li>\n                    <li data-amount="2">\n                        <b>PRAVDA 2</b><span>2&euro;</span>\n                    </li>\n                    <li data-amount="5">\n                        <b>PRAVDA 5</b><span>5&euro;</span>\n                    </li>\n                    <li data-amount="10">\n                        <b>PRAVDA 10</b><span>10&euro;</span>\n                    </li>\n                </ul>\n                <div class="text-center">\n                    <img src="data:image/gif;base64,R0lGODlhAQABAAAAACwAAAAAAQABAAA=" class="qr-image" />\n                    <div>\n                        <small>Cena spätnej SMS je vo zvolenej sume vrátane DPH.<br>Prevádzkovateľom služby je ADMINA s. r. o.</small>\n                    </div>\n                </div>\n            </div>\n            <div class="modal-footer">\n                <a href="sms:8877;?&amp;body=PRAVDA%205" class="btn btn-primary">Poslať SMS</a>\n                <button type="button" class="btn btn-default" data-dismiss="modal">Zavrieť</button>\n            </div>\n        </div>\n    </div>\n</div>').appendTo('body');$modalSms.find('ul li').on('click',function(){$(this).addClass('active').siblings().removeClass('active');var txt=encodeURIComponent('PRAVDA '+$(this).attr('data-amount'));$modalSms.find('.qr-image').attr('src','https://chart.googleapis.com/chart?cht=qr&chs=150x150&chld=Q&chl=SMSTO:8877:'+txt);$modalSms.find('a.btn-primary').attr('href','sms:8877;?&body='+txt);});});})(jQuery);(function($){$.fn.PerexPoll=function(){$(this).each(function(){var helperData=PEREX.getCookieNamespace('tx_perex_poll'),isVotingEnabled=true,now=Math.round(+new Date()/ 1000),pollUid=$(this).attr('data-id'),$container=$(this),$containerForm=$(this).find('form'),$containerResults=$(this).find('.article-detail-anketa-vysledky');if(helperData){helperData=helperData.split(',');if($.inArray(pollUid+'',helperData)>-1){$containerForm.html('<div class="alert alert-success">Ďakujeme za Váš hlas.</div>');$containerResults.show();isVotingEnabled=false;}}
$(this).on('click','.show-results-button',function(e){e.preventDefault();$(this).children('span').toggleClass('glyphicon-menu-up','glyphicon-menu-down');$containerResults.slideToggle('slow');});$(this).on('click','.btn-submit',function(){$containerForm.submit();});$(this).on('change','input:radio',function(){if(isVotingEnabled){$container.find('input[type="submit"], .btn-submit').removeAttr('disabled');}});$containerForm.submit(function(e){e.preventDefault();if($containerForm.hasClass('sending')){return false;}else{$containerForm.addClass('sending');}
var ajaxData=PEREX.Helper.getFormValues($containerForm);ajaxData['ajax']='tx_perex_poll';ajaxData['tx_perex_poll_pi1[uid]']=pollUid;$.ajax({url:$containerForm.attr('action'),data:ajaxData,type:'POST',dataType:'json',success:function(jsonData){var $results=$('.article-detail-anketa[data-id="'+pollUid+'"] .article-detail-anketa-vysledky');$('.article-detail-anketa[data-id="'+pollUid+'"] form').html('<div class="alert alert-success">Ďakujeme za Váš hlas.</div>');for(var i in jsonData.options){var option=jsonData.options[i];var $row=$results.find('.anketa-row[data-option="'+option.sorting+'"] .anketa-row-chart');$row.attr('title',option.votes+' '+PEREX.Helper.plural(option.votes,'hlas','hlasy','hlasov'));$row.children('.anketa-row-chart-text').text(option.relative.toFixed(1).replace('.',',')+'%');$row.children('.anketa-row-chart-vect').css({width:option.relative+'%'});}
$results.slideDown();},error:function(jqXHR,textStatus,errorThrown){$containerForm.removeClass('sending').append('<div class="alert alert-danger" role="alert">Nastala chyba.</div>');}});return false;});});};PEREX.registerInitHook(function(){$('.perex-poll').PerexPoll();})})(jQuery);(function($){$.fn.PerexToplist=function(){this.each(function(){var options={ajaxId:$(this).attr('data-ajax-id'),ajaxLink:$(this).attr('data-ajax-url')}
...
PEREX.registerInitHook(function(){$('.perex-toplist').PerexToplist();});})(jQuery);(function($){$.fn.PerexGallery=function(settings){var defaults={currentImage:0,reloadPolicy:true,enableKeyboardNavigation:true,galleryData:{},referrer:'',url:'',$containerContent:null,$containerContentPhoto:null,$containerImage:null,$containerDescription:null,$containerAuthor:null,$containerItems:null,$containerAdvertisement:null,$containerCloseButton:null,$containerThumbnailsButton:null};$.extend(this,{normalizeHash:function(hash){return hash.replace(/^.*#/,'').replace(/\?.*$/,'');},getIndexFromHash:function(){var hash=parseInt(this.normalizeHash(document.location.hash));return(!isNaN(hash)?hash:1);},changeHash:function(index){document.location.hash='#'+index;window.location.replace((''+window.location).split('#')[0]+'#'+index);if(this.reloadPolicy){window.location.reload();}},next:function(){this.changeHash(this.getNextIndex(this.currentImage));},previous:function(){this.changeHash(this.getPreviousIndex(this.currentImage));},getNextIndex:function(index){var nextIndex=index+1;if(nextIndex>this.imagesTotal){nextIndex=1;}
return nextIndex;},getPreviousIndex:function(index){var prevIndex=index-1;if(prevIndex<=0){prevIndex=this.imagesTotal;}
...
if(this.currentImage!==index){var imageData=this.galleryData[index-1];this.openImage(imageData);}
this.currentImage=index;if(!supressHashChange&&this.getIndexFromHash()!==index){this.changeHash(index);}
if(!this.reloadPolicy){var preloadImage=new Image();preloadImage.src=this.galleryData[this.getNextIndex(index)-1].url;}},openImage:function(imageData){var self=this;$('<img>').attr({src:imageData.url,alt:imageData.description}).on('load',function(){self.$containerImage.replaceWith(this);self.$containerImage=$(this);self.updateImageCaption(imageData);});$(window).scrollTop(0);},updateImageCaption:function(imageData){this.$containerDescription.text(imageData.description);this.$containerAuthor.text(imageData.author);this.$containerAuthor.append($('<span>').html('<i>'+this.currentImage+'</i>/'+this.galleryData.length));}});var gallery=this;$.extend(this,defaults,settings);if(!this.url.length){this.url=document.location.toString().split('#')[0];}
this.imagesTotal=settings.galleryData.length;this.$containerContent=$(this).find('.pravda-fotogaleria-open-cont');this.$containerContentPhoto=$(this).find('.pravda-fotogaleria-open-cont-photo');this.$containerImage=this.$containerContentPhoto.children('img');this.$containerDescription=this.$containerContentPhoto.children('.pravda-fotogaleria-open-info');this.$containerAuthor=this.$containerContentPhoto.children('.pravda-fotogaleria-open-autor');this.$containerItems=this.find('.pravda-fotogaleria-open-items');this.$containerAdvertisement=$('#perex-gallery-advertisement');this.$containerCloseButton=$(this).find('.gallery-close');this.$containerThumbnailsButton=$(this).find('.gallery-thumbnails');if(this.normalizeHash(document.location.hash)==="thumbnails"){gallery.$containerContent.slideUp(0);}else{this.gotoIndex(this.getIndexFromHash(),true);}
$(window).bind('hashchange',function(){var index=gallery.getIndexFromHash();if(gallery.currentImage===index)
return;if(gallery.reloadPolicy){window.location.reload();}else{gallery.gotoIndex(index);}});$(this).find('.gallery-navigation').click(function(e){e.preventDefault();if($(this).hasClass('prev')){gallery.previous();}else{gallery.next();}});if(this.enableKeyboardNavigation){$(document).keydown(function(e){var key=e.charCode?e.charCode:e.keyCode?e.keyCode:0;switch(key){case 32:break;case 33:break;case 34:break;case 35:gallery.gotoIndex(gallery.imagesTotal);e.preventDefault();break;case 36:gallery.gotoIndex(1);e.preventDefault();break;case 37:gallery.previous();e.preventDefault();break;case 39:gallery.next();e.preventDefault();break;}});}
...
if(this.$containerCloseButton&&(typeof sessionStorage==="object")){var helperData=sessionStorage.getItem('tx_perex_gallery')?jQuery.parseJSON(sessionStorage.getItem('tx_perex_gallery')):null;if(document.referrer.length&&(document.referrer.search(this.url)===-1)){this.referrer=document.referrer;}
else if((helperData!==null)&&(typeof helperData[$(this).attr('id')]!=='undefined')){this.referrer=helperData[$(this).attr('id')].link;}else{}
if(this.referrer!==''){var obj=new Object;obj[$(this).attr('id')]={'link':this.referrer,'time':new Date().getTime()}
helperData=$.extend(helperData,obj);sessionStorage.setItem('tx_perex_gallery',PEREX.Helper.stringify(helperData));}
this.$containerCloseButton.attr('href',this.referrer);}
if(this.$containerThumbnailsButton){this.$containerThumbnailsButton.click(function(e){e.preventDefault();gallery.$containerContent.slideToggle();gallery.$containerItems.toggleClass('items-expanded');});}
if('scrollRestoration'in history){history.scrollRestoration='manual';}};})(jQuery);(function($){$.fn.PerexListIntervals=function(){this.each(function(){var self=this;$(self).find('button[data-interval]').click(function(e){e.preventDefault();$(this).addClass('active').siblings('button[data-interval]').removeClass('active');var $list=$(self).find('.interval-list-'+$(this).attr('data-interval'));if($list.length){$list.find('img.lazy').trigger('appear');$list.addClass('active').siblings('[class*="interval-list-"]').removeClass('active');}
if(typeof ga==="function"){var parentName=$(self).attr('id')?$(self).attr('id'):'unknown';ga('send','event','Interval',parentName,$(this).text());ga('b.send','event','Interval',parentName,$(this).text());}});});};PEREX.registerInitHook(function(){$('.perex-intervals').PerexListIntervals();});})(jQuery);(function($){$.fn.PerexQACompetition=function(){$(this).each(function(){var $form=$(this);var $errorsContainer=$form.find('.article-detail-sutaz-errors');var $submitButton=$form.find('input[type="submit"]');$form.submit(function(e){e.preventDefault();try{$errorsContainer.find('.alert').remove();var errors=[];if(!$('#interview-name').val()){errors.push('Nezadali ste svoje meno.');}
...
$form.find('input[type="radio"]').attr('disabled','disabled');$form.find('.user-form-evaluation').html('<div class="alert alert-info text-center" role="alert">'+userFormEvaluationText+'</div>');$('<input>').attr({type:'button',value:'Pochváľ sa na Facebooku!',class:'btn btn-lg btn-share-full'}).click(function(e){e.preventDefault();FB.ui({method:'feed',link:document.location.href,description:userFormEvaluationText+' Aký výsledok budeš mať ty?'},function(){if(typeof ga==="function"){ga('send','event','Share','Kviz','vysledok kvizu');ga('b.send','event','Share','Kviz','vysledok kvizu');}});}).appendTo($form.find('.user-form-evaluation'));};$form.submit(function(){try{if(questionCount>$form.find(':radio[class="ans"]:checked').length){alert('Musíte odpovedať na všetky otázky');return false;}
...
return false;});$(document).on('click','input[name="share"]',function(e){e.preventDefault();FB.ui({method:'share',href:document.location.href},function(){if(typeof ga==="function"){ga('send','event','Share','Známkovanie','známkovanie');ga('b.send','event','Share','Známkovanie','známkovanie');}});});};})(jQuery);(function($){$.fn.PerexVote=function(initialData){var self=this,$container=$(this),$buttons=$container.find('.buttons'),sending=false,helperData=PEREX.getCookieNamespace('tx_perex_vote_pi1'),isVotingEnabled=true,voteId=$(this).find('input[name="tx_perex_vote_pi1[uid]"]').val(),highcharts=[];var ajaxOptions={url:$container.attr("action")||"ajax/",type:$container.attr("method")||"post",dataType:'json'};$.extend(this,{showResults:function(data){while(highcharts.length){highcharts.pop().destroy();}
...
if(typeof FB==="object"){$('#article-fb-share').click(function(e){e.preventDefault();FB.ui({method:'share',href:document.location.href},function(){if(typeof ga==="function"){ga('send','event','Share','Hlasovanie','hlasovanie');ga('b.send','event','Share','Hlasovanie','hlasovanie');}});});}};})(jQuery);(function($){var Helper={validatorEmail:function(val){return(/^("([ !\x23-\x5B\x5D-\x7E]*|\\[ -~])+"|[-a-z0-9!#$%&'*+\/=?^_`{|}~]+(\.[-a-z0-9!#$%&'*+\/=?^_`{|}~]+)*)@([0-9a-z\u00C0-\u02FF\u0370-\u1EFF]([-0-9a-z\u00C0-\u02FF\u0370-\u1EFF]{0,61}[0-9a-z\u00C0-\u02FF\u0370-\u1EFF])?\.)+[a-z\u00C0-\u02FF\u0370-\u1EFF][-0-9a-z\u00C0-\u02FF\u0370-\u1EFF]{0,17}[a-z\u00C0-\u02FF\u0370-\u1EFF]$/i).test(val);}};$.fn.PerexNewsletterBox=function(){$(this).each(function(){var form=$(this);var sending=false;var ajaxOptions={url:form.attr("action")||"ajax/",type:form.attr("method")||"post",dataType:'json'};form.submit(function(e){e.preventDefault();var sendValues=PEREX.Helper.getFormValues(form);if(sending){return;}
sending=true;form.fadeTo('fast',0.2);$.ajax($.extend(ajaxOptions,{data:sendValues,success:function(data){if(typeof data.newsletter!="undefined"){form.parent().html(data.newsletter);}else{if(typeof data.status!="undefined"){alert(data.status);}
sending=false;form.fadeTo('fast',1);}}}));});form.find('input[type="submit"]').on('click',function(e){e.preventDefault();var sendValues=PEREX.Helper.getFormValues(form);if(!Helper.validatorEmail(sendValues['email'])){alert('Pre prihlásenie spravodajstva musíte zadať svoju e-mailovú adresu.');return;}
form.find('.newsletter-box-gdpr').modal('show');});form.find('.newsletter-box-gdpr .btn-primary').on('click',function(){form.submit();});});}
$.extend({PerexNewsletter:{init:function(container,ajaxLink){var checkboxTimeout;var service=PEREX.Helper.getURLParameter('service');var hash=PEREX.Helper.getURLParameter('hash');var cmd=PEREX.Helper.getURLParameter('CMD');if(hash!='null'&&cmd!='null'){$.ajax({data:{'ajax':'newsletter','CMD':cmd,'service':service,'hash':hash},url:ajaxLink,type:"post",dataType:'json',success:function(data){container.find('.loading').hide();if(typeof data.newsletter!="undefined"){container.html(data.newsletter).trigger('refreshed');}else{container.find('form').show();if(typeof data.status!="undefined"){alert(data.status);}}}});}else{container.find('.loading').hide();container.find('form').show();}
$(container).on('click','.service-switch :checkbox',function(evt){var checkbox=this;clearTimeout(checkboxTimeout);checkboxTimeout=setTimeout(function(){$(checkbox).parent().siblings('.loading').css('visibility','visible');var cmd=$(checkbox).is(':checked')?'subscribe':'unsubscribe';var service=$(checkbox).attr('data-service');var hash=$(checkbox).attr('data-hash');$.ajax({data:{'ajax':'newsletter','CMD':cmd,'service':service,'hash':hash},url:ajaxLink,type:"post",dataType:'json',success:function(data){if(typeof data.newsletter!="undefined"){container.html(data.newsletter).trigger('refreshed');}else if(typeof data.status!="undefined"){alert(data.status);}}});},700);});$(container).on('refreshed',function(){$(this).find('.services-list .notify').each(function(i,self){setTimeout(function(){$(self).fadeOut('fast');},3000);});});}}});$(document).ready(function(){$('form.perex-newsletter').PerexNewsletterBox();});})(jQuery);(function($){$.fn.PerexSticky=function(target){this.each(function(){if($(this).next().length){return;}
var $container=$(this);var $wrapper=$('<div class="sticky-locator"></div>').insertBefore($container);var $content=typeof target==='undefined'?$wrapper.parents('.right-column').prev('.content-column'):$(target);$(window).scroll(function(){var position=$wrapper.offset();var headerHeight=parseInt($('#sticky-menu').length?$('#sticky-menu').outerHeight()+20:20);var contentHeight=parseInt($content.innerHeight()-$container.outerHeight(true));if((position.top<contentHeight)&&($(this).scrollTop()>position.top-headerHeight)){if($(this).scrollTop()<contentHeight+$content.offset().top-headerHeight){$container.css({position:'fixed',top:headerHeight,left:position.left-$(window).scrollLeft()});}else{$container.css({position:'absolute',top:contentHeight,left:0});}}else{$container.css({position:'relative',top:0,left:0});}}).resize(function(){$(this).scroll();}).scroll();});};$(window).on('load',function(){$('.perex-sticky, .advert-bottomsquare').PerexSticky();});})(jQuery);(function($){$.fn.PerexSlide=function(){this.each(function(){var $container=$(this);var $wrap=$(this).find('.slide-wrap');var left=0;var innerWidth=0;var resizeTimeout=null;$container.find('.slide-item').each(function(){innerWidth+=$(this).outerWidth(true);});function translateX(px){$wrap.css({'-webkit-transform':'translateX('+px+'px)','-moz-transform':'translateX('+px+'px)','-ms-transform':'translateX('+px+'px)','transform':'translateX('+px+'px)'});}
...
if(left<=-innerWidth+$container.width()){$container.find('.slide-btn-next').addClass('inactive');}else{$container.find('.slide-btn-next').removeClass('inactive');}}).trigger('resize');$(window).on('resize',function(){clearTimeout(resizeTimeout);resizeTimeout=setTimeout(function(){if($wrap.parent().css('overflow-x')==='scroll'){left=0;translateX(left);}else{$container.trigger('resize');}},500);});});};$(window).on('load',function(){$('.perex-slide').PerexSlide();});})(jQuery);(function($){$.fn.PerexCalendar=function(){this.each(function(){var container=this;var options={ajaxId:$(container).attr('data-ajax-id'),ajaxLink:$(container).attr('data-ajax-url'),prefixId:$(container).attr('data-prefix-id')}
jQuery(container).on('click','.intervals a[rel="nofollow"]',function(e){e.preventDefault();e.stopPropagation();$(this).html('').addClass('glyphicon glyphicon-refresh glyphicon-animate');var ajaxData={ajax:options.ajaxId};ajaxData[options.prefixId+'[year]']=$(this).attr('data-year');ajaxData[options.prefixId+'[month]']=$(this).attr('data-month');if($(container).attr('data-section')){ajaxData[options.prefixId+'[section]']=$(container).attr('data-section');}
return $.ajax({url:options.ajaxLink,type:"post",data:ajaxData,dataType:'json',success:function(data,textStatus,jqXHR){$(container).html(data);}});});});};PEREX.registerInitHook(function(){$('.perex-calendar').PerexCalendar();});})(jQuery);(function($){window.PerexFlashnewsUpdate={};$.fn.PerexFlashnews=function(){this.each(function(){var self=this;var $container=$(this);var $refreshButton=$container.find('.btn-refresh');var $buttonLeft=$container.find(".hp-box-bleskove-spravy-menu-left");var $buttonRight=$container.find(".hp-box-bleskove-spravy-menu-right");var $list=$container.children('ul');if(!$list.length){$list=$('<ul>').appendTo($container);}
$.extend(this,{page:0,pageCount:0,promotedCount:parseInt($(this).attr('data-count')),sectionId:parseInt($(this).attr('data-id')),oneStep:$(window).width()<768?1:3,refreshInterval:null,autoScrollInterval:parseInt($(this).attr('data-interval')),scrollInterval:null,next:function(){if(self.page<self.pageCount-1){self.page++;self.repairTransforms();self.repairButtons();self.resetAutoScrollInterval();}},previous:function(){if(self.page>0){self.page--;self.repairTransforms();self.repairButtons();self.resetAutoScrollInterval();}},repairButtons:function(){self.pageCount=Math.floor($container.find("li").length / self.oneStep);if(self.page<=0){$buttonLeft.addClass('invisible');}else{$buttonLeft.removeClass('invisible');}
if(self.page>=self.pageCount-1){$buttonRight.addClass('invisible');}else{$buttonRight.removeClass('invisible');}},repairTransforms:function(animate){var newLeft=-(self.page*self.oneStep*parseInt($container.find("li").first().outerWidth(true)));if(typeof animate==='undefined'||animate){$container.find("li").animate({'left':newLeft},{duration:500,queue:false});}else{$container.find("li").css('left',newLeft);}},createItem:function(data){var $li=$('<li>').addClass('col-md-5 col-sm-5 col-xs-15').attr('data-id',data.id).hide();$('<div>').addClass('article-added-info').append($('<span>').text(data.formattedTime).addClass('article-added-info-date')).appendTo($li);var $paragraph=$('<p>').appendTo($li);if(data.url){$('<a>').attr('href',data.url).html((data.title?'<b>'+data.title+'</b>':data.text)+" ").append($('<span>').addClass('glyphicon glyphicon-new-window')).appendTo($paragraph);if(data.title){$('<span>').text(" "+data.text).appendTo($paragraph);}}else{if(data.title){$('<b>').text(data.title+" ").appendTo($paragraph);}
$paragraph.append($('<span>').text(data.text));}
return $li;},listUpdate:function(jsonData){var ids=[];if(self.refreshInterval){clearInterval(self.refreshInterval);}
self.refreshInterval=setInterval(self.refresh,60e3);if(typeof jsonData!=="object"||!jsonData.length){return;}
$list.find('li').each(function(){ids.push(parseInt($(this).attr('data-id')));$(this).remove();});for(var i in jsonData){var item=jsonData[i];if(ids.indexOf(item.id)<0){$list.append(self.createItem(item).fadeIn('slow'));}else{$list.append(self.createItem(item).show());}}
self.repairTransforms();self.repairButtons();self.resetAutoScrollInterval();},refresh:function(){$refreshButton.children('span').addClass('glyphicon-animate');setTimeout(function(){$refreshButton.children('span').removeClass('glyphicon-animate');},750);$.getScript("//ipravda."+PEREX.Helper.getPravdaTLD()+"/xml/flashnews-"+self.sectionId+".js");},autoScroll:function(){if($buttonRight.hasClass('invisible')){self.page=0;self.repairTransforms();self.repairButtons();}else{self.next();}},resetAutoScrollInterval:function(){if(self.scrollInterval){clearInterval(self.scrollInterval);}
if(!$container.is(':hover')){self.scrollInterval=setInterval(self.autoScroll,self.autoScrollInterval*1000);}}});$(window).resize(function(){var newOneStep=$(window).width()<768?1:3;if(self.oneStep>newOneStep){self.page=self.page*self.oneStep;}else if(self.oneStep<newOneStep){self.page=Math.floor(self.page / newOneStep);}
if(self.oneStep!==newOneStep){self.oneStep=newOneStep;self.repairButtons();}
self.repairTransforms(false);});$(".hp-box-bleskove-spravy-menu-left").click(this.previous);$(".hp-box-bleskove-spravy-menu-right").click(this.next);if(!isNaN(self.sectionId)){window.PerexFlashnewsUpdate[self.sectionId]=self.listUpdate;$refreshButton.click(function(e){e.preventDefault();self.page=0;self.refresh();}).click();}else{self.repairButtons();}
if(typeof jQuery.fn.swipe!='undefined'){$(this).swipe({swipe:function(event,direction,distance,duration,fingerCount,fingerData){if(direction=='left'){self.next();}
if(direction=='right'){self.previous();}},allowPageScroll:"vertical"});}
if(!isNaN(self.autoScrollInterval)){$container.on('mouseenter',function(){clearInterval(self.scrollInterval);}).on('mouseleave',function(){self.scrollInterval=setInterval(self.autoScroll,self.autoScrollInterval*1000);});}});};$.fn.PerexFlashnewsV2=function(){this.each(function(){var self=this;var $container=$(this);var $refreshButton=$container.find('.btn-refresh');var $list=$container.find('ul');if(!$list.length){$list=$('<ul>').appendTo($container);}
$.extend(this,{sectionId:parseInt($(this).attr('data-id')),refreshInterval:null,createItem:function(data){var $li=$('<li>').addClass('col-lg-15 col-xs-15').attr('data-id',data.id).hide();$('<span>').text(PEREX.Helper.timeAgo(new Date(data.time))).addClass('article-added-info-date').appendTo($li);var $paragraph=$('<p>').appendTo($li);if(data.url){$('<a>').attr('href',data.url).html((data.title?'<b>'+data.title+'</b>':data.text)+" ").append($('<span>').addClass('glyphicon glyphicon-new-window')).appendTo($paragraph);if(data.title){$('<span>').text(" "+data.text).appendTo($paragraph);}}else{if(data.title){$('<b>').text(data.title+" ").appendTo($paragraph);}
$paragraph.append($('<span>').text(data.text));}
return $li;},listUpdate:function(jsonData){var ids=[];if(self.refreshInterval){clearInterval(self.refreshInterval);}
self.refreshInterval=setInterval(self.refresh,30e3);if(typeof jsonData!=="object"||!jsonData.length){return;}
$list.find('li').each(function(){ids.push(parseInt($(this).attr('data-id')));$(this).remove();});for(var i in jsonData){var item=jsonData[i];if(ids.indexOf(item.id)<0){$list.append(self.createItem(item).fadeIn('slow'));}else{$list.append(self.createItem(item).show());}}},refresh:function(){$refreshButton.children('span').addClass('glyphicon-animate');setTimeout(function(){$refreshButton.children('span').removeClass('glyphicon-animate');},2500);$.getScript("//ipravda."+PEREX.Helper.getPravdaTLD()+"/xml/flashnews-"+self.sectionId+".js");}});if(!isNaN(self.sectionId)){window.PerexFlashnewsUpdate[self.sectionId]=self.listUpdate;$refreshButton.click(function(e){e.preventDefault();self.refresh();}).click();}});};$.fn.PerexFlashnewsList=function(){this.each(function(){setInterval(function(){location.reload();},60e3);$(this).find('.btn-refresh').click(function(e){$(this).children('.glyphicon-refresh').addClass('glyphicon-animate');});});};PEREX.registerInitHook(function(){$('.perex-flashnews').PerexFlashnews();$('.perex-flashnews-v2').PerexFlashnewsV2();$('.perex-flashnews-box').PerexFlashnewsList();});})(jQuery);